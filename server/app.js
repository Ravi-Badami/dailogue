...
try {
  ...
  app.post('/api/register', async (req, res, next) => {
    try {
      ...
      if (!fullName || !email || !password) {
        ...
        res.status(400).send('Please fill all required fields of registration');
      } else {
  ...
  app.post('/api/login', async (req, res, next) => {
    try {
      ...
      if (!email || !password) {
        res.status(400).send('Please fill the all required fields of login');
      } else {
        ...
        if (!user) {
          ...
          if (!validateUser) {
            ...
            const JWT_SCERET_KEY = 'This_is_a_JWT_Secret_key';
